...
void toErrorLog(const std::string& msg) {
	std::ofstream pFile("error.log", std::ios_base::app);
	pFile << msg.c_str() << std::endl;
//	std::cout << msg << "\n";
}
...
{
	std::ofstream pFile("init.log", std::ios_base::app);
	pFile << msg.c_str() << std::endl;
...
const bool parse_simple_line(std::string& text, std::list<std::string>& words)
{
	size_t n = text.length();
	size_t stop;
	bool in_parentheses=false;
...
const bool loadStringFile(const std::string& string_file, const /*std::vector<std::string>&*/std::string* string_identifier, /*std::vector< std::vector<std::string> >&*/ std::string* string_list, /*std::vector<bool>& language_loaded,*/ const unsigned int max_strings)
{
