...
void MyActor::rotate(float y_angle_rad) {
	...
	speed -= (speed * diff / 3.141f);
	strafe -= (strafe * diff / 3.141f);
}
...
void MyActor::move(CTVLandscape* my_land, float time_elapsed) {
	pPosition->x += (cosf(pDirection->y) * speed * time_elapsed) + (cosf(pDirection->y + 3.141596f / 2.0f) * strafe * time_elapsed);
	pPosition->z += (sinf(pDirection->y) * speed * time_elapsed) + (sinf(pDirection->y + 3.141596f / 2.0f) * strafe * time_elapsed);
	pPosition->y = my_land->GetHeight(pPosition->x, pPosition->z) + 2.0f;

	pActor->SetRotation(0.0f, 270.0f + pDirection->y * (-180.0f / 3.141f), 0.0f);
	pActor->SetPosition( pPosition->x, pPosition->y, pPosition->z);
}
